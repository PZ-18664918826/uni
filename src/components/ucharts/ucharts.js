...
function getSeriesDataItem(series, index, group) {
  const data = [];
  let newSeries = [];
  const indexIsArr = index.constructor.toString().indexOf('Array') > -1;
  if (indexIsArr) {
...
function getCandleToolTipData(series, seriesData, opts, index, categories, extra) {
  ...
  const { calPoints } = opts.chartData;
...
function findBarChartCurrentIndex(currentPoints, calPoints, opts, config) {
  ...
  const { yAxisPoints } = opts.chartData;
  if (calPoints && calPoints.length > 0) {
    if (isInExactChartArea(currentPoints, opts, config)) {
...
function findRoseChartCurrentIndex(currentPoints, pieData, opts) {
  ...
  if (pieData && pieData.center && isInExactPieChartArea(currentPoints, pieData.center, pieData.radius)) {
    ...
    if (opts.extra.rose && opts.extra.rose.offsetAngle) {
      angle -= (opts.extra.rose.offsetAngle * Math.PI) / 180;
    }
...
function findPieChartCurrentIndex(currentPoints, pieData, opts) {
  ...
  if (pieData && pieData.center && isInExactPieChartArea(currentPoints, pieData.center, pieData.radius)) {
    ...
    if (opts.extra.pie && opts.extra.pie.offsetAngle) {
      angle -= (opts.extra.pie.offsetAngle * Math.PI) / 180;
    }
    if (opts.extra.ring && opts.extra.ring.offsetAngle) {
      angle -= (opts.extra.ring.offsetAngle * Math.PI) / 180;
    }
    for (let i = 0, len = series.length; i < len; i++) {
...
function calCategoriesData(categories, opts, config, eachSpacing, context) {
  ...
  const fontSize = opts.xAxis.fontSize * opts.pix || config.fontSize;
  ...
  if (opts.xAxis.rotateLabel == true) {
    ...
    tempHeight =
      tempHeight < fontSize + 2 * config.xAxisTextPadding ? tempHeight + 2 * config.xAxisTextPadding : tempHeight;
...
function calTooltipYAxisData(point, series, opts, config, eachSpacing) {
  ...
  for (let i = 0; i < ranges.length; i++) {
    ...
    item = opts.yAxis.data && opts.yAxis.data[i].formatter ? opts.yAxis.data[i].formatter(item) : item.toFixed(0);
...
function drawPieText(series, opts, config, context, radius, center) {
  ...
  const seriesConvert = series.map(function (item, index) {
...
function drawMarkLine(opts, config, context) {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    if (item.showLabel) {
      ...
      context.setFontSize(config.fontSize);
      const textWidth = measureText(labelText, config.fontSize, context);
      const bgWidth = textWidth + config.toolTipPadding * 2;
      ...
      const textX = bgStartX + config.toolTipPadding;
      const textY = item.y;
...
function drawColumnDataPoints(series, opts, config, context) {
  const process = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 1;
  const { xAxisData } = opts.chartData;
  const { xAxisPoints } = xAxisData;
  const { eachSpacing } = xAxisData;
...
function drawBubbleDataPoints(series, opts, config, context) {
  ...
  series.forEach(function (eachSeries, seriesIndex) {
    ...
    if (opts.dataLabel !== false && process === 1) {
      points.forEach(function (item, index) {
        context.beginPath();
        const fontSize = series.textSize * opts.pix || config.fontSize;
        context.setFontSize(fontSize);
...
function drawXAxis(categories, opts, config, context) {
  const { xAxisData } = opts.chartData;
  const { xAxisPoints } = xAxisData;
  const { startX } = xAxisData;
  const { endX } = xAxisData;
  const { eachSpacing } = xAxisData;
  let boundaryGap = 'center';
  if (
    opts.type == 'bar' ||
  ...
  if (opts.xAxis.disabled !== true) {
    ...
    if (config._xAxisTextAngle_ === 0) {
      ...
      newCategories.forEach(function (item, index) {
        ...
        const offsetY =
          startY + 6 * opts.pix + xAxisFontSize - xAxisFontSize * Math.abs(Math.sin(config._xAxisTextAngle_));
...
function drawYAxisGrid(categories, opts, config, context) {
  ...
  const startX = opts.area[3];
  const { xAxisPoints } = opts.chartData.xAxisData;
...
function drawCharts(type, opts, config, context) {
  ...
  if (
    ...
    const { xAxisHeight } = _calCategoriesData;
    const { angle } = _calCategoriesData;
  ...
  if (opts.enableScroll && opts.xAxis.scrollAlign == 'right' && opts._scrollDistance_ === undefined) {
    let offsetLeft = 0;
    const { xAxisPoints } = opts.chartData.xAxisData;
    const { startX } = opts.chartData.xAxisData;
    const { endX } = opts.chartData.xAxisData;
    const { eachSpacing } = opts.chartData.xAxisData;
    ...
    case 'line':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          const { xAxisPoints } = _drawLineDataPoints;
          const { calPoints } = _drawLineDataPoints;
          const { eachSpacing } = _drawLineDataPoints;
    ...
    case 'scatter':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          const { xAxisPoints } = _drawScatterDataPoints;
          const { calPoints } = _drawScatterDataPoints;
          const { eachSpacing } = _drawScatterDataPoints;
          opts.chartData.xAxisPoints = xAxisPoints;
          opts.chartData.calPoints = calPoints;
          opts.chartData.eachSpacing = eachSpacing;
          drawYAxis(series, opts, config, context);
    ...
    case 'bubble':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          const { xAxisPoints } = _drawBubbleDataPoints;
          const { calPoints } = _drawBubbleDataPoints;
          const { eachSpacing } = _drawBubbleDataPoints;
          opts.chartData.xAxisPoints = xAxisPoints;
          opts.chartData.calPoints = calPoints;
          opts.chartData.eachSpacing = eachSpacing;
          drawYAxis(series, opts, config, context);
    ...
    case 'mix':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          const { xAxisPoints } = _drawMixDataPoints;
          const { calPoints } = _drawMixDataPoints;
          const { eachSpacing } = _drawMixDataPoints;
          opts.chartData.xAxisPoints = xAxisPoints;
          opts.chartData.calPoints = calPoints;
          opts.chartData.eachSpacing = eachSpacing;
          drawYAxis(series, opts, config, context);
    ...
    case 'column':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          const { xAxisPoints } = _drawColumnDataPoints;
          const { calPoints } = _drawColumnDataPoints;
          const { eachSpacing } = _drawColumnDataPoints;
          opts.chartData.xAxisPoints = xAxisPoints;
          opts.chartData.calPoints = calPoints;
          opts.chartData.eachSpacing = eachSpacing;
          drawYAxis(series, opts, config, context);
    ...
    case 'mount':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          const { xAxisPoints } = _drawMountDataPoints;
          const { calPoints } = _drawMountDataPoints;
          const { eachSpacing } = _drawMountDataPoints;
          opts.chartData.xAxisPoints = xAxisPoints;
          opts.chartData.calPoints = calPoints;
          opts.chartData.eachSpacing = eachSpacing;
          drawYAxis(series, opts, config, context);
    ...
    case 'bar':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          const { yAxisPoints } = _drawBarDataPoints;
          const { calPoints } = _drawBarDataPoints;
          const { eachSpacing } = _drawBarDataPoints;
    ...
    case 'area':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          const { xAxisPoints } = _drawAreaDataPoints;
          const { calPoints } = _drawAreaDataPoints;
          const { eachSpacing } = _drawAreaDataPoints;
          opts.chartData.xAxisPoints = xAxisPoints;
          opts.chartData.calPoints = calPoints;
          opts.chartData.eachSpacing = eachSpacing;
          drawYAxis(series, opts, config, context);
    ...
    case 'candle':
      this.animationInstance = new Animation({
        ...
        onProcess: function onProcess(process) {
          ...
          const { xAxisPoints } = _drawCandleDataPoints;
          const { calPoints } = _drawCandleDataPoints;
          const { eachSpacing } = _drawCandleDataPoints;
          opts.chartData.xAxisPoints = xAxisPoints;
          opts.chartData.calPoints = calPoints;
          opts.chartData.eachSpacing = eachSpacing;
          drawYAxis(series, opts, config, context);
...
uCharts.prototype.updateData = function () {
  ...
  switch (scrollPosition) {
    ...
    case 'right':
      ...
      const { yAxisWidth } = _calYAxisData;
      this.config.yAxisWidth = yAxisWidth;
      let offsetLeft = 0;
      const _getXAxisPoints0 = getXAxisPoints(this.opts.categories, this.opts, this.config);
      const { xAxisPoints } = _getXAxisPoints0;
      const { startX } = _getXAxisPoints0;
      const { endX } = _getXAxisPoints0;
      const { eachSpacing } = _getXAxisPoints0;
      const totalWidth = eachSpacing * (xAxisPoints.length - 1);
      const screenWidth = endX - startX;
      offsetLeft = screenWidth - totalWidth;
      this.scrollOption = {
...
uCharts.prototype.dobuleZoom = function (e) {
  ...
  if (!this.scrollOption.moveCount) {
    ...
    if (this.opts.rotate) {
      cts0 = {
        changedTouches: [{ x: this.opts.height / this.opts.pix - this.opts.area[0] / this.opts.pix - 2, y: tcs[0].y }]
      };
      cts1 = {
        changedTouches: [{ x: this.opts.height / this.opts.pix - this.opts.area[0] / this.opts.pix - 2, y: tcs[1].y }]
      };
    }
...
uCharts.prototype.showToolTip = function (e) {
  ...
  const _touches$ = getTouches(touches, this.opts, e);
  var { currentOffset } = this.scrollOption;
  ...
  if (
    ...
    if (index > -1 || index.length > 0) {
      ...
      if (seriesData.length !== 0) {
        ...
        var { textList } = _getToolTipData;
        var { offset } = _getToolTipData;
        offset.y = _touches$.y;
        opts.tooltip = {
  ...
  if (this.opts.type === 'mix') {
    ...
    if (index > -1) {
      ...
      if (seriesData.length !== 0) {
        ...
        var { textList } = _getMixToolTipData;
        var { offset } = _getMixToolTipData;
        offset.y = _touches$.y;
        opts.tooltip = {
          textList: option.textList ? option.textList : textList,
          offset: option.offset !== undefined ? option.offset : offset,
...
uCharts.prototype.scroll = function (e) {
  ...
  if (touches && this.opts.enableScroll === true) {
    ...
    const { currentOffset } = this.scrollOption;
...
uCharts.prototype.scrollEnd = function (e) {
  if (this.opts.enableScroll === true) {
    ...
    const { currentOffset } = _scrollOption;
    const { distance } = _scrollOption;
